...
function* tryParseCustomMetric(event, dimensions, timestamp) {
	try {
		log.info('CUSTOM METRIC', event)
		const match = regex.exec(event)
		if (!match) {
			return
		}
		log.info('CUSTOM METRIC matched', event)
